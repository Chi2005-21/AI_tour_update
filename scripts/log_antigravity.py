...
def get_gemini_home() -> Path:
    ...
    if user_profile:
        p = Path(user_profile) / ".gemini"
        if p.exists():
            return p

    # 3. If in WSL, try querying cmd.exe to find USERPROFILE
    try:
        win_profile = subprocess.check_output(
            ["cmd.exe", "/c", "echo %USERPROFILE%"],
            shell=False, text=True, stderr=subprocess.DEVNULL
        ).strip()
        if win_profile and ":" in win_profile:
            try:
                wsl_profile = subprocess.check_output(
                    ["wslpath", win_profile],
                    shell=False, text=True, stderr=subprocess.DEVNULL
                ).strip()
                p = Path(wsl_profile) / ".gemini"
                if p.exists():
                    return p
            except Exception:
                drive = win_profile[0].lower()
                rest = win_profile[2:].replace("\\", "/")
                p = Path(f"/mnt/{drive}{rest}") / ".gemini"
                if p.exists():
                    return p
    except Exception:
        pass

    # 4. Fallback: Traverse parents of CWD to find Windows Users folder
    try:
        cwd_parts = Path.cwd().parts
        if "Users" in cwd_parts:
            idx = cwd_parts.index("Users")
            if idx + 1 < len(cwd_parts):
                p = Path(*cwd_parts[:idx + 2]) / ".gemini"
                if p.exists():
                    return p
    except Exception:
        pass
